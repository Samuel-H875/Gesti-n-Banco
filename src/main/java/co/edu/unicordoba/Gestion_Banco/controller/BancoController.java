...
public class BancoController {
    ...
    private final EmpleoRepository empleos;
    private final BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();

    public BancoController(ClienteRepository clientes, CuentaRepository cuentas,
                           EmpleadoRepository empleados, EmpleoRepository empleos) {
    ...
    public String crearEmpleado(@RequestParam String nombre, @RequestParam String documento,
                               @RequestParam String correo, @RequestParam String cargo, HttpSession s) {
        if (s.getAttribute("admin") == null) return "redirect:/login";
        var empleo = empleos.findByNombre(cargo);
        if (empleo.isPresent()) {
            empleados.save(new Empleado(nombre, documento, correo, cargo, empleo.get().getSalario()));
        }
        return "redirect:/admin";
    }

    @PostMapping("/admin/empleo")
    public String crearEmpleo(@RequestParam String nombre, @RequestParam double salario, HttpSession s) {
        if (s.getAttribute("admin") == null) return "redirect:/login";
        if (!nombre.isBlank() && salario >= 0 && !empleos.existsByNombre(nombre)) {
            empleos.save(new Empleo(nombre.trim(), salario));
        }
        return "redirect:/admin";
    }

    @PostMapping("/admin/empleo/editar")
    public String editarEmpleo(@RequestParam String nombreAnterior, @RequestParam String nombre,
                               @RequestParam double salario, HttpSession s) {
        if (s.getAttribute("admin") == null) return "redirect:/login";
        if (!nombre.isBlank() && salario >= 0) {
            empleos.findByNombre(nombreAnterior).ifPresent(e -> {
                String cargoAnterior = e.getNombre();
                if (!cargoAnterior.equalsIgnoreCase(nombre) && empleos.existsByNombre(nombre)) return;
                empleos.update(nombreAnterior, nombre.trim(), salario);
                empleados.findAll().stream()
                        .filter(emp -> emp.getCargo().equalsIgnoreCase(cargoAnterior))
                        .forEach(emp -> { emp.setCargo(nombre.trim()); emp.setSalario(salario); });
            });
        }
        return "redirect:/admin";
    }

    @PostMapping("/admin/empleo/eliminar")
    public String eliminarEmpleo(@RequestParam String nombre, HttpSession s) {
        if (s.getAttribute("admin") == null) return "redirect:/login";
        boolean usado = empleados.findAll().stream()
                .anyMatch(e -> e.getCargo().equalsIgnoreCase(nombre));
        if (!usado) empleos.deleteByNombre(nombre);
        return "redirect:/admin";
    }
    ...
    private void cargarAdmin(Model m) {
        ...
        m.addAttribute("empleos", empleos.findAll());
    }
